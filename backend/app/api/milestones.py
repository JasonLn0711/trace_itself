...
from app.api.deps import get_milestone_or_404, require_auth
...
router = APIRouter(prefix="/milestones", tags=["milestones"], dependencies=[Depends(require_auth)])


def ensure_project_exists(project_id: int, db: Session) -> None:
...
def list_milestones(
    ...
    stmt = select(Milestone).order_by(Milestone.due_date.asc().nulls_last(), Milestone.created_at.desc())
...
def upcoming_milestones(limit: int = 10, db: Session = Depends(get_db)) -> list[Milestone]:
    stmt = (
        select(Milestone)
        .where(Milestone.due_date >= date.today(), Milestone.status != MilestoneStatus.COMPLETED)
...
def create_milestone(payload: MilestoneCreate, db: Session = Depends(get_db)) -> Milestone:
    ensure_project_exists(payload.project_id, db)
...
def update_milestone(
    ...
    if "project_id" in changes:
        ensure_project_exists(changes["project_id"], db)
